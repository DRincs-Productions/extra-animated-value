    ...
    def __init__(self, value_object, *images, **kwargs):
        ...
        for idx, i in enumerate(self.images):
            if not isinstance(i, (list, tuple)):
                self.images[idx] = [i, (float(idx) * (1.0 / (len(self.images) - 1)))]
    ...
    def render(self, width, height, st, at):
        ...
        for img, perc in self.images:
            if perc <= self.old_value:
                ...
                if self.dissolve:
                    ratio = float(self.old_value - use_perc) / (perc - use_perc)

                    if ratio > 0.01:
                        layers.append(
                            renpy.render(
                                Transform(img, alpha=ratio), width, height, st, at
                            )
                        )
        ...
        if getattr(self.value_object, "value") != getattr(
            self.value_object, "current_value"
        ):
            renpy.redraw(self, 0)
